...
const STRING HTTP_RESPONSE_VERSION = "HTTP/1.0 ";
...
public:
   ...
   // virtual VOID process_POST();
    ...
    //virtual VOID process_CGI_writereqbody(FILEFD fd, const STRING& reqbody);
   // virtual VOID process_CGI_readresponse(FILEFD fd);
   // virtual VOID process_CGI_setoneenv(const STRING& key, const WORD32& val);
   // virtual VOID process_CGI_setoneenv(const STRING& key, const STRING& header);
