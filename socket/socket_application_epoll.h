...
{
public:
    CSocketApplicationEpoll(STRING serverip, WORD16 serverport, CSocketRequest* req) :\
                             CSocketApplication(serverip, serverport, req){}
    virtual VOID working();
    virtual VOID epoll_setevent(WORD32 events, SOCKFD fd, VOID* req, SWORD32 op);
    virtual VOID epoll_self(WORD32 events, CSocketRequest* req);
    virtual VOID epoll_peer(WORD32 events, CSocketRequest* req);
protected:
    SOCKFD  m_epfd;
    EPEVENT m_epev;
